TokoVoipConfig = {
	...
	wsServer = "170.81.41.74:33250", -- Address of the websocket server
	...
	plugin_data = {
		...
		TSChannel = "[JOGANDO]",
		TSPassword = "FuNNyGay24*@", -- TeamSpeak channel password (can be empty)
		...
		TSServer = "diamondcity.ryzenhost.com.br", -- TeamSpeak server address to be displayed on blocking screen
...
AddEventHandler("onClientResourceStart", function(resource)
	if (resource == GetCurrentResourceName()) then	--	Initialize the script when this resource is started
		Citizen.CreateThread(function()
			TokoVoipConfig.plugin_data.localName = escape("[ DIAMOND CITY ]");
		end);
