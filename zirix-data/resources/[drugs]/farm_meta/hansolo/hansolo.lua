...
Citizen.CreateThread(function()
	while true do
		...
		if not servico then			
			...
			local distance = GetDistanceBetweenCoords(config.start[1],config.start[2],cdz,x,y,z,true)
...
Citizen.CreateThread(function()
	while true do
		...
		if servico then
			local ped = PlayerPedId()
			if selec == false then
				selecionado = math.random(#config.locs)
				selec = true
			end
			local x,y,z = table.unpack(GetEntityCoords(ped))
			local bowz,cdz = GetGroundZFor_3dCoord(config.locs[selecionado].x,config.locs[selecionado].y,config.locs[selecionado].z)
			local distance = GetDistanceBetweenCoords(config.locs[selecionado].x,config.locs[selecionado].y,cdz,x,y,z,true)			
			...
			if distance <= 15 then
				idle = 5
				DrawMarker(23,config.locs[selecionado].x,config.locs[selecionado].y,config.locs[selecionado].z-0.90,0, 0, 0, 0, 0, 0, 0.7, 0.7, 0.5, 136, 96, 240, 180, 0, 0, 0, 0)
				if distance <= 1.5 then
					drawTxt("PRESSIONE  ~r~E~w~  PARA COLETAR O PACOTE",4,0.5,0.93,0.50,255,255,255,180)
					if IsControlJustPressed(0,38) then
						if farm_meta.checkPayment() then
							
							porcentagem = math.random(1,100)

							if porcentagem >= config.percentage[1] and porcentagem <= config.percentage[2] then
...
function CriandoBlip(locs,selecionado)
	blips = AddBlipForCoord(config.locs[selecionado].x,config.locs[selecionado].y,config.locs[selecionado].z)
