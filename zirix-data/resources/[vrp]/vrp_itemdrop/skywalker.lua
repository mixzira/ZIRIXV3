...
AddEventHandler('DropSystem:take33', function(id)
	...
	if nuser_id then
		...
		if user_id then
			...
			if verifyAmount ~= '' and verifyAmount <= items[id].count then
				local myinventory = {}
				...
				if items[id] ~= nil and tSlot > 0 then
					local new_weight = vRP.getInventoryWeight(user_id)+vRP.getItemWeight(items[id].item)*parseInt(verifyAmount)
					if new_weight <= vRP.getInventoryMaxWeight(user_id) then
								...
								{ 
									...
									fields = {
										...
										{ 
											...
											value = '[ **Item: '..items[id].name..'** ][ **Quantidade: '..parseInt(verifyAmount)..'** ]\n⠀⠀'
										},
										{ 
						...
						TriggerClientEvent('itensNotify', source, 'sucesso', 'Pegou', ''..items[id].item..'', ''..parseInt(verifyAmount)..'', ''..vRP.getItemWeight(items[id].item)*parseInt(verifyAmount)..'')
