...
function farm_coca.checkPayment()
	...
	if user_id then
		if vRP.getInventoryWeight(user_id)+vRP.getItemWeight(config2.itens[1])*qtd <= vRP.getInventoryMaxWeight(user_id) then
			vRP.giveInventoryItem(user_id,config2.itens[1],qtd)
			TriggerClientEvent("Notify",source,"sucesso","Você coletou "..qtd.." de <b>"..config2.itens[1].."</b>.")
			if vRP.getInventoryWeight(user_id)+vRP.getItemWeight(config2.itens[2])*qtd <= vRP.getInventoryMaxWeight(user_id) then
				vRP.giveInventoryItem(user_id,config2.itens[2],qtd)
				TriggerClientEvent("Notify",source,"sucesso","Você coletou "..qtd.." de <b>"..config2.itens[2].."</b>.")
				if vRP.getInventoryWeight(user_id)+vRP.getItemWeight(config2.itens[3])*qtd <= vRP.getInventoryMaxWeight(user_id) then
					vRP.giveInventoryItem(user_id,config2.itens[3],qtd)
					TriggerClientEvent("Notify",source,"sucesso","Você coletou "..qtd.." de <b>"..config2.itens[3].."</b>.")
				else
